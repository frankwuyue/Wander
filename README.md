...
- [ ] **Server-side API proxy** — Move Anthropic API call to a backend function. Protect the key.
- [ ] **PWA icons** — Create proper icon set (192, 512, maskable). Current placeholder SVG won't install cleanly.
- [ ] **Offline graceful degradation** — Show cached last session or a static "thinking" prompt if offline
- [ ] **iOS safe area padding** — Handle notch/home indicator with `env(safe-area-inset-*)` 
...
- [ ] **Onboarding flow** — First-time user experience explaining the philosophy before first session
- [ ] **Share a question** — After flushing, option to share just the *carry question* (not full content) as an image. Keeps privacy intact while enabling organic growth.

### Phase 3 — Depth
- [ ] **Session history search** — Full-text search within the Question Journal
- [ ] **Annual review** — A "year in questions" summary generated by AI from your journal at year end
- [ ] **Mood tagging** — After flushing, a single emoji tap for how you felt. No numeric scores. Enables future pattern analysis.
- [ ] **Dead Genius mode** — New session type: Take a real problem (from seed) and run it through the lens of a historical thinker (Marcus Aurelius, Buckminster Fuller, etc.)
- [ ] **"The Detour" session type** — A random walk: starts with one idea, makes 3 unexpected hops, lands somewhere completely different
